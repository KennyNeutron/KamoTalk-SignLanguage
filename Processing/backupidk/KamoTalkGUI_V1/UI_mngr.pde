...
void PrintButton(int buttonAddress) {
  ...
  case TRANSLATE:
    if (!pwMatch()) {
      CreateButton_Large((width/2)-300, 150, #D3D3D3, Color_BLACK, "TRANSLATE", Font_Default_Bold, Color_BLACK, 40);
    } else {
  ...
  case ADD_GESTURE:
    if (!pwMatch()) {
      CreateButton_Large((width/2)-300, 300, #D3D3D3, Color_BLACK, "ADD HANDSIGN", Font_Default_Bold, Color_BLACK, 40);
    } else {
